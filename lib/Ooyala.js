...
function Ooyala (apiKey, apiSecretKey, expireInMinutes) {
  ...
  this._expireIn = expireInMinutes || 120;
}
...
OO._request = function(method, url, file, onSuccess, onError) {
  if(onSuccess != null){
    this._curl(method, this._buildPath(method, path, params, body), body, onSuccess, onError);
  }else{
    return new Promise(function(resolve, reject){ 
      this._curl(method, this._buildPath(method, path, params, body), body, resolve, reject);
    }.bind(this));
  }
}
